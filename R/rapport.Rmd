...
enqueteur_par_entretien <- interview_complete(df = suso_actions) %>%
    dplyr::semi_join(cases_to_review, by = c("interview__id", "interview__key")) |>
    dplyr::rename_with(
        .cols = dplyr::matches("_last_complete"),
        .fn = ~ stringr::str_replace(.x, pattern = "_last_complete", replacement = "")
    ) |>
    dplyr::mutate(
        # date d'achèvement
...
equipe_enqueteur_entretien <- enqueteur_par_equipe %>%
    dplyr::left_join(enqueteur_par_entretien, by = "interviewer")
...
gps_stats_by_overall <- gps_stats_counts %>%
    dplyr::summarise(
        dplyr::across(
            .cols = c(n_entretiens, n_parcelles, n_non_gps, petit, panne, indisponible, loin, autre),
            .fns = ~ sum(.x, na.rm = TRUE)
        )
    ) %>%
    dplyr::mutate(
