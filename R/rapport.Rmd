...
interview_metadata <- menages |>
    ...
    dplyr::select(
        # identifiants
        interview__id, interview__key, zd,
        # achèvement
        is_complete,
        # date
        date, week, 
...
zd_du_rapport <- dplyr::case_when(
    params$type_enquete == "transversale" ~ list(1:585),
    params$type_enquete == "panel" ~ list(586:1185),
    TRUE ~ list(1:585)
) |> unlist()

cases_to_keep <- interview_metadata |>
    dplyr::filter(is_complete == TRUE) |>
    dplyr::filter(zd %in% zd_du_rapport)
...
# trier la base communautaire
community <- community |>
    dplyr::filter(!!rlang::data_sym(params$grappe_var) %in% zd_du_rapport)
...
equipes <- susoapi::get_supervisors(workspace = params$workspace) %>%
    {
        if (!"FullName" %in% names(.)) {
            dplyr::mutate(., FullName = NA_character_)
        } else if ("FullName" %in% names(.)) {
            .
        }
    } %>%
    dplyr::mutate(
        supervisor = UserName,
        supervisor_name = dplyr::if_else(
            condition = !is.na(FullName),
            true = FullName,
            false = UserName
        )
    ) %>%
    dplyr::select(supervisor, supervisor_name)
...
equipe_enqueteur_entretien <- enqueteur_par_equipe %>%
    ...
    dplyr::mutate(supervisor = supervisor_name) |>
    dplyr::select(-supervisor_name)
...
reponse_globale_counts <- equipe_enqueteur_entretien %>%
    dplyr::filter(report_period %in% c(TRUE, NA) & (zd %in% zd_du_rapport)) %>%
...
taille_menage_by_interview <- equipe_enqueteur_entretien %>%
    ...
    dplyr::group_by(interview__id, interview__key, supervisor, zd) %>%
    dplyr::summarise(
...
```{r eval_panel}
is_panel <- (params$type_e == "panel")
```

```{r hhold_size_panel, eval=is_panel}
